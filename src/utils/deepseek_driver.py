...
def get_conversation_messages(driver: Driver) -> list:
    """
    Get all messages from the current web conversation.
    ...
    try:
        ...
        message_elements = driver.find_elements("xpath", "//div[contains(@class, 'ds-markdown')]")
        ...
        for element in message_elements:
            try:
                ...
                # Process the HTML to get clean text
                clean_content = _basic_html_cleanup(content)
                if not clean_content or not clean_content.strip():
                    continue

                # Try to determine if this is a user or assistant message
                # Look at the parent container or surrounding elements for role indicators
                parent = element.find_element("xpath", "./..")
                parent_classes = parent.get_attribute("class") or ""

                # DeepSeek typically uses different styling for user vs assistant messages
                # This is a heuristic and might need adjustment based on actual DOM structure
                if "user" in parent_classes.lower():
                    role = "user"
                elif "assistant" in parent_classes.lower():
                    role = "assistant"
                else:
                    # Default heuristic: messages alternate, first is usually user
...
def is_conversation_synced(driver: Driver, st_messages: list) -> bool:
    """
    Check if the web conversation is synced with SillyTavern messages.

    Args:
        driver: WebDriver instance
        st_messages: List of messages from SillyTavern

    Returns:
        bool: True if conversations are synced, False otherwise
    """
    try:
        web_messages = get_conversation_messages(driver)

        # If web is empty but ST has messages, not synced
        if not web_messages and st_messages:
            return False

        # If both are empty, they're synced
        if not web_messages and not st_messages:
            return True

        # Quick check: if message counts are very different, not synced
        if abs(len(web_messages) - len(st_messages)) > 2:  # Allow some tolerance
            return False

        # Check the last few messages for content similarity
        # We'll compare the last 2-3 messages to allow for some variation
        check_count = min(3, len(web_messages), len(st_messages))
        ...
        for i in range(1, check_count + 1):
            web_msg = web_messages[-i] if i <= len(web_messages) else None
            st_msg = st_messages[-i] if i <= len(st_messages) else None

            if web_msg is None or st_msg is None:
                continue

            # Compare content similarity (allowing for minor differences)
            web_content = web_msg['content'].strip().lower()
            st_content = st_msg.get('content', '').strip().lower()

            # Remove common formatting differences
            web_content = re.sub(r'\s+', ' ', web_content)
            st_content = re.sub(r'\s+', ' ', st_content)

            # Simple similarity check - if they're very different, not synced
            if len(web_content) > 10 and len(st_content) > 10:
                # Check if one contains most of the other (allowing for formatting differences)
                shorter = min(web_content, st_content, key=len)
                longer = max(web_content, st_content, key=len)

                # If the shorter text is substantially contained in the longer, consider it a match
                if len(shorter) > 20:  # Only for non-trivial messages
                    # Simple substring check with some tolerance
                    if shorter[:50] not in longer and longer[:50] not in shorter:
                        return False

        return True

    except Exception as e:
        print(f"Error checking conversation sync: {e}")
        # If we can't determine, assume not synced to be safe
        return False

def is_conversation_empty(driver: Driver) -> bool:
    """
    Check if the current conversation in the web interface is empty (no messages).

    Returns:
        bool: True if conversation is empty, False if it has messages
    """
    try:
        # Look for message containers in the conversation
        # DeepSeek uses ds-markdown class for message content
        messages = driver.find_elements("xpath", "//div[contains(@class, 'ds-markdown')]")

        # If no message elements found, conversation is empty
        if not messages:
            return True

        # Check if any messages have actual content (not just empty divs)
        for message in messages:
            content = message.get_attribute("innerHTML")
            if content and content.strip():
                # Found a message with content
                return False

        # All message elements are empty
        return True

    except Exception as e:
        print(f"Error checking if conversation is empty: {e}")
